...
func (si *SyncTime) GetCurrentDayTimestamp() int64 {
	now := time.Now()
	day := now.Day()
	month := now.Month()
	year := now.Year()
...
func (si *SyncTime) GetSyncTimestamp() int64 {
	unixTimestamp := si.GetCurrentDayTimestamp()
	...
	for {
		...
		for {
			...
			if notEmpty {
				...
				// TODO: In the main thread, a similar functionality occurs given a similar kind of if, so this code doesn't run
